...
import {ContractNegotiationDto, NegotiationId, NegotiationInitiateRequestDto} from "../../edc-dmgmt-client";
...
export class ContractNegotiationService {
  ...
  finishedNegotiations: Map<string, ContractNegotiationDto> = new Map<string, ContractNegotiationDto>();
  ...
  constructor(
    ...
    if (!environment.production) {
      ...
      this.runningNegotiations.set("offer:6db25605-cd8c-4528-ade2-6a90349d06ac", {} as any)
      this.finishedNegotiations.set("offer:6db25605-cd8c-4528-ade1-6a90389d06ac", {} as any)
    }
  }

  isBusy(contractOffer: ContractOffer) {
    return this.runningNegotiations.get(contractOffer.id) !== undefined
      || !!this.runningTransferProcesses.find(tp => tp.assetId === contractOffer.asset.id);
  }
  ...
  negotiate(contractOffer: ContractOffer) {
    ...
    this.catalogBrowserService.initiateNegotiation(initiateRequest).subscribe(negotiationId => {
      ...
      if (!this.pollingHandleNegotiation) {
        ...
        this.pollingHandleNegotiation = setInterval(() => {
          for (const negotiation of this.runningNegotiations.values()) {
            this.catalogBrowserService.getNegotiationState(negotiation.id).subscribe(updatedNegotiation => {

...
public class SovityDataPlaneManagerImpl extends AbstractStateEntityManager<DataFlow, DataPlaneStore> implements DataPlaneManager {
    ...
    private boolean processReceived(DataFlow dataFlow) {
        ...
        return entityRetryProcessFactory.doAsyncProcess(dataFlow, () -> transferService.transfer(request))
                ...
                .onFailure((f, t) -> {
                    transitionDataFlowToReceived(f, runtimeId);
                    update(f);
                })
    ...
    @SneakyThrows
    private void transitionDataFlowToReceived(DataFlow dataFlow, String runtimeId) {
        // When transitioning to RECEIVED, the runtimeId needs to be set as well, since it may have changed after a restart of the EDC.
        var runtimeIdField = DataFlow.class.getDeclaredField("runtimeId");
        runtimeIdField.setAccessible(true);

        runtimeIdField.set(dataFlow, runtimeId);
        dataFlow.transitToReceived();
    }

    public static class Builder extends AbstractStateEntityManager.Builder<DataFlow, DataPlaneStore, SovityDataPlaneManagerImpl, Builder> {

        private Builder() {
            super(new SovityDataPlaneManagerImpl());
        }
        ...
        @Override
        public SovityDataPlaneManagerImpl build() {
            super.build();

...
-- Empty example migration
-- This directory will contain all our Community Edition migrations in the future.
-- Commercial Edition migrations will have to be compatible version-wise.
-- This file is added to prevent flyway from crashing due no migrations.
SELECT 1;
-- Statements are designed for and tested with Postgres only!
...
UPDATE edc_transfer_process SET updated_at=created_at;

ALTER TABLE edc_transfer_process ALTER COLUMN updated_at SET NOT NULL;
ALTER TABLE edc_transfer_process ALTER COLUMN created_at SET NOT NULL;
--
--  Copyright (c) 2023 sovity GmbH
--
--  This program and the accompanying materials are made available under the
...
UPDATE edc_contract_definitions SET created_at=EXTRACT(EPOCH FROM CURRENT_TIMESTAMP) * 1000;
...
UPDATE edc_asset SET created_at=EXTRACT(EPOCH FROM CURRENT_TIMESTAMP) * 1000;
...
--
--
ALTER TABLE edc_transfer_process ADD transferprocess_properties JSON;
--
--  Copyright (c) 2023 sovity GmbH
--
--  This program and the accompanying materials are made available under the
...
--
--
UPDATE edc_transfer_process SET transferprocess_properties = '{}'::json WHERE transferprocess_properties IS NULL;
--
--  Copyright (c) 2023 sovity GmbH
--
--  This program and the accompanying materials are made available under the

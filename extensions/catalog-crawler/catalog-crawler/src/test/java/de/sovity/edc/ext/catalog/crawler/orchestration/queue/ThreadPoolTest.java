...
class ThreadPoolTest {
    ...
    void testParallelExecution() {
        ...
        var aRef = mock(ConnectorRef.class);
        var bRef = mock(ConnectorRef.class);
        ...
        threadPool.enqueueConnectorRefreshTask(0, a, aRef);
        threadPool.enqueueConnectorRefreshTask(0, b, bRef);

        assertThat(threadPool.getQueuedConnectorRefs()).containsExactlyInAnyOrder(aRef, bRef);
    ...
    private ThreadPool newThreadPool(int numThreads) {
        ...
        var crawlerConfig = mock(CrawlerConfig.class);
        when(crawlerConfig.getNumThreads()).thenReturn(numThreads);
        var queue = new ThreadPoolTaskQueue();
        return new ThreadPool(queue, crawlerConfig, monitor);
    }

...
const TransferHistoryPage = () => {
  const pageQuery = useQueryWrapper(
    queryKeys.transferHistory.key(),
    () => api.uiApi.getTransferHistoryPage(),
    {refetchInterval: 5000},
  );

  const t = useTranslations();
  useTitle(t('Pages.TransferHistory.title'));
  const breadcrumbItems = useBreadcrumbItems();
  useBreadcrumbs([breadcrumbItems.transferHistoryList()]);
  ...
  return matchQueryState({
    ...
    success: (data) => (
      <PageContainer>
        <TransferHistoryTable data={data.transferEntries} />
      </PageContainer>
    ),
  });
};

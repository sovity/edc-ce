...
export class AssetDetailDialog implements OnDestroy {
  ...
  private confirmDelete(): Observable<boolean> {
    const dialogData = ConfirmDialogModel.forDelete("asset", `"${this.asset.name}"`)
    const ref = this.matDialog.open(ConfirmationDialogComponent, {maxWidth: "20%", data: dialogData});

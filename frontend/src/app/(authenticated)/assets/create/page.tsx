...
import {dataOfferFormValueDontPublish} from '@/app/(authenticated)/data-offers/create/components/data-offer-form-value-builder';
...
export default function DataOfferCreatePage() {
  ...
  useTitle(t('Pages.AssetCreate.title'));
  useBreadcrumbs([
    breadcrumbItems.assets.listPage(),
    breadcrumbItems.assets.createPage(),
  ]);
  ...
  return (
    <PageContainer>
      <DataOfferForm
        mode={'CREATE_ASSET'}
        initialFormValue={dataOfferFormValueDontPublish()}
        onSubmit={onSubmit}
      />
    </PageContainer>
  );

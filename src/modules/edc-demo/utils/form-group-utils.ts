...
>(opts: {
  ...
  const allValidators = Object.values(opts.validators) as Partial<
    Record<keyof T, any>
  >[];
  const fields = [
    ...new Set(allValidators.flatMap((it) => Object.keys(it))),
  ] as (keyof T)[];

  const updateDatasourceValidators = () => {
    // Remove all validators
    fields.map(ctrl).forEach((control) => {
    ...
    const validators = Object.entries(
      opts.validators[opts.switchCtrl.value],
    ) as [keyof T, ValidatorFn | ValidatorFn[]][];
    validators.forEach(([key, validators]) =>
      ctrl(key).setValidators(validators),
    );
  };
